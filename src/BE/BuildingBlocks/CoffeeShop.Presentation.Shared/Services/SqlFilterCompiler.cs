...
namespace CoffeeShop.Presentation.Shared.Services
{
    public class SqlFilterCompiler : IFilterCompiler
    {
        ...
        public Tuple<string, object[], string[]> Compile(JObject filter, ref int count, Action<string[]> callback = null)
        {
            if (filter.Count != 1)
            {
                var filterModel = filter.ToObject<FilterModel>(JsonConstant.JsonSerializer);
                if (!supportOperations.ContainsKey(filterModel.Operation))
                    throw new NotSupportedException($"{filterModel.Operation} is not supported");
            ...
            {
                var dictionary = filter.ToObject<IDictionary<string, JArray>>(JsonConstant.JsonSerializer);
                ...
                foreach (var item in dictionary)
                {
                    foreach (var value in item.Value)
                    {
                        ...
                        if (rs.Item2.Length != rs.Item3.Length)
